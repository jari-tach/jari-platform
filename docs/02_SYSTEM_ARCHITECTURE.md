...
> ⚠️ **نطاق هذا المستند:** يصف هذا المستند العمارة الداخلية لتطبيق **SAEQ Driver** فقط، وهو أحد أربع تطبيقات مستقلة تُشكّل منصة SAEQ (SAEQ Driver، SAEQ Customer، SAEQ Merchant، SAEQ Admin). كل تطبيق مستقل بمشروعه، وPackage Name، ودورة Build/Test/Release الخاصة به، ويتشارك مع باقي التطبيقات فقط في البنية الخلفية المشتركة (Backend، API Gateway، المصادقة، قاعدة البيانات المركزية). راجع القرار المعماري الكامل في [ADR-013: Separate Applications Strategy](./adr/ADR_SEPARATE_APPLICATIONS_STRATEGY.md). أنماط العمارة النظيفة والتنظيم Feature-First الموصوفة أدناه تنطبق داخل كل تطبيق من التطبيقات الأربعة على حدة، دون أي دمج للأدوار في تطبيق واحد.
